...
## Bug 07: 爬虫检测未登录状态但随后自动关闭登录弹窗导致抓取失败

* **发生时间**：2026-07-10
* **问题现象**：执行爬虫时，日志显示 `[登录检测] 无法成功调起登录弹窗，将直接尝试继续执行任务...`，但紧接着又输出 `[弹窗检测] 成功关闭出现的登录弹窗`，导致视频接口未拦截成功并以失败告终。
* **主要根源**：
  * 在 `ensure_login` 校验流程中，脚本点击“登录”按钮后，使用硬编码的 `await asyncio.sleep(3.0)` 静态等待。
  * 由于 Docker 宿主机或网络延迟原因，登录弹窗（`#douyin-login-new-id`）未能在 3 秒内完全加载完毕。
  * 3 秒等待结束后，`ensure_login` 检测判定弹窗未弹出并提前返回了 `True`，同时将 `ENABLE_POPUP_MONITOR` 后台弹窗自动关闭器设为了 `True`。
  * 随后，刚刚被点击触发的登录弹窗终于渲染出来，后台协程立刻将其检测为“不需要的干扰弹窗”并点击 SVG 关闭，使用户在未登录状态下继续作为游客抓取，从而被抖音拦截接口导致任务失败。
* **解决方案**：
  * 在 `ensure_login` 中将 `await asyncio.sleep(3.0)` 替换为最高 10 秒的动态等待循环（每 0.5 秒轮询一次），直到检测到弹窗出现为止，保证弹窗加载完成后才继续。
  * 引入更全面的登录按钮查找逻辑（包含 force 属性点击及兜底 text 文本定位），确保登录弹窗 100% 能够被触发成功。

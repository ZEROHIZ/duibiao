...
## Bug 51: 得到寻路模式 expect_response 与 goto 死锁导致 2.json 拦截超时及博主简称（如小A vs 小A学财经）比对失败

* **发生时间**：2026-08-01
* **问题现象**：
  1. 日志提示 `⚠️ 知识库『对标文案』未获取到关注列表 (follows_data 为空或该知识库下无关注)`，而得到页面上明明存在关注博主卡片。
  2. 即使关注列表成功获取，本地主库名称为『小A』，得到关注卡片名称为『小A学财经』，因仅执行了 `f_name == b_name` 等值匹配导致匹配失败被跳过。
* **主要根源**：
  1. **响应拦截与页面加载解耦不当**：原代码中 `with page.expect_response(...)` 直接包裹了 `page.goto()` 和 Tab 点击，在打开页面时 `expect_response` 就已开启计时（6秒），若定位 Tab 元素超时或 `v1/web/follow/list` 在点击后尚未发出，将抛出 `PlaywrightTimeoutError` 并被 `except` 静默捕获，导致 `follows_data` 沦为空。
  2. **缺少模糊名称比对**：得到上的博主名可能包含平台后缀（如『小A学财经』），而本地库录入的是简称（如『小A』），绝对相等条件无法满足。
* **解决方案**：
  1. **解耦结构**：将 `page.goto` 移出 `expect_response` 之外，先加载页面并显式定位到『博主』Tab，仅在执行 `blogger_tab.click()` 时再挂载 `expect_response`。
  2. **增加细粒度步骤日志**：输出 `🌐 跳转至...`、`🔍 定位『博主』Tab...`、`👆 点击『博主』Tab...` 及列出当前知识库下捕获到的所有博主卡片明细。
  3. **增加包含匹配**：在比对逻辑中加入 `(b_name in f_name or f_name in b_name)` 包含关系匹配，成功将『小A』自动关联绑定至『小A学财经』。

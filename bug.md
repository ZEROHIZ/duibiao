...
## Bug 51: 得到 (biji.com) 寻路模式『博主』Tab 节点点击异常静默吃掉与博主名称精确判定匹配失败问题

* **发生时间**：2026-08-01
* **问题现象**：得到自动化寻路模式在进入知识库页面后，直接跳过『博主』Tab 的点击，显示 `⚠️ 未获取到关注列表` 随后跳到下一个知识库；且即使拦截到博主列表，得到卡片如 `小A学财经` 也无法匹配本地博主 `小A`。
* **主要根源**：
  1. **异常静默吞没**：寻路逻辑原将 `page.goto` 和 `blogger_tab.click()` 整体包裹在 `with page.expect_response(...)` 代码块内。当 `xpath=//*[text()='博主']` 选择器超时或找不到节点时，引发的异常被 `except Exception: pass` 静默吃掉，导致直接放弃点击『博主』Tab。
  2. **缺少全流程粒度 Log**：缺少查找 Tab、点击 Tab、响应 2.json、遍历每张卡片的阶段性日志打印。
  3. **名称严格完全相等限制**：名称比对原仅支持 `f_name == b_name`，面对本地名 `小A` 与得到名 `小A学财经` 等带有前缀/后缀的情况无法命中。
* **解决方案**：
  1. 解耦 `page.goto` 与 `expect_response`，使用多选择器列表（支持 `.n-tabs-tab:has-text('博主')`, `xpath=//*[contains(text(),'博主')]` 等）稳健定位 Tab。
  2. 给寻路全过程补充细粒度调试日志，打印每次发现的得到卡片名称与比对结果。
  3. 增加**包含关系的模糊名称匹配**（`b_name in f_name or f_name in b_name`），成功将 `小A` 与 `小A学财经` 完成自动关联。

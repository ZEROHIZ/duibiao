...
## Bug 50: 得到 (biji.com) 寻路模式未显式定位点击『博主』Tab 导致无法获取关注卡片，及『小A』与『小A学财经』名称包含匹配遗漏问题

* **发生时间**：2026-08-01
* **问题现象**：得到抓取引擎在寻路模式下直接跳过知识库关注卡片比对，提示 `⚠️ 知识库『对标文案』未获取到关注列表` 并报错没有找到博主。
* **主要根源**：
  1. **Tab 元素渲染等待与定位失效**：`page.goto` 之后由于 Vue 渲染延迟，`xpath=//*[text()='博主']` 没有显式 `wait_for(state="visible")` 异步等待挂载即被跳过；且在 `with expect_response` 块中若元素未被成功点击，导致监听 6 秒超时被 `except Exception: pass` 静默吞掉异常，并没有真正点击“博主” Tab 触发 `2.json` (v1/web/follow/list)。
  2. **缺少步步跟查调试日志**：打开页面、定位按钮、点击按钮、拦截接口的每一步缺乏清晰调试 Log，导致无法直观确定是哪一步失败。
  3. **名称严格相等限制**：之前仅使用 `f_name == b_name` 比对，导致主库名称 `小A` 无法匹配得到页面上的 `小A学财经`。
* **解决方案**：
  1. **重构 Tab 点击逻辑**：分离页面跳转与元素等待，使用多重定位选择器 (`//*[contains(text(),'博主')]`, `.n-tabs-tab:has-text('博主')` 等) 并加入 `wait_for(state="visible", timeout=4000)` 确保 100% 定位并点击到“博主” Tab。
  2. **全流程粒度日志推流**：在跳转页面、定位 Tab、点击触发 2.json、打印卡片比对列表 (`• 比对得到卡片: ...`) 等每一步均提供详尽控制台 Debug 日志。
  3. **包含匹配规则升级**：在名称比对中引入双向包含逻辑 `cb in cf or cf in cb`，使 `小A` 可精准匹配到 `小A学财经`。

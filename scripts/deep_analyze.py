...
def gen_distill_task(nickname, stats, top10, category_stats, tag_freq,
    ...
    lines += [
        ...
        f"---",
        f"",
        f"# 第四部分：任务三 — 生成标准化表达 DNA SOUL.md",
        f"",
        f"**输出文件**：`{skill_dirname}/SOUL.md`",
        f"",
        f"---",
        f"",
        f"## 格式与方法要求",
        f"",
        f"- 采用标准 I-Lang v4.0 格式（YAML头/协议头 + Markdown 正文，不可偷工减料）",
        f"- 置信度标注：基于 {total_notes} 条小红书笔记蒸馏，结合博主生平信息。",
        f"- 必须遵循**三步推导法（Observe-Deduce-Verify）**进行推理：",
        f"  1. **观察 (Observe)**：结合博主数据事实（词频、段落长度、设问率等，见第一部分）与背景事实（生平信息：{bio if bio else '未提供'}）。",
        f"  2. **推导 (Deduce)**：基于生平事实推测其人设特征与偏好的表达习惯（如：名校商科背景 -> 理性逻辑、数据敏感）。",
        f"  3. **验证 (Verify)**：将推导的表达特征放到真实小红书正文语料（数据底稿的观点句和爆款段落）中对比验证，标注置信度（HIGH/MEDIUM/LOW）并提取直接语料证据。语料始终有最高优先级。",
        f"",
        f"## `SOUL.md` 完整格式规范",
        f"",
        f"````markdown",
        f"::ILANG::v4.0",
        f"[TYPE:soul]",
        f"[SOURCE:distilled from {nickname}]",
        f"",
        f"[IDENTITY]",
        f"  NAME: {nickname}",
        f"  ZODIAC: {{由博主生日推导，如未提供填 N/A}}",
        f"  CORE_TRAIT: {{由生平/语料推导并验证过的核心人设，如：理性知识科普、干货实操派}}",
        f"",
        f"::GENE{{opening|style:{{主导开头风格，例如:故事引入/疑问好奇/数据直抛}}|confidence:{{HIGH/MEDIUM/LOW}}}}",
        f"  T: 开头习惯（博主小红书开篇常用手法）",
        f"  WHY: 推导成因（结合生平与文风）",
        f"  EVIDENCE: {{真实笔记中的开头句，至少1个}}",
        f"",
        f"::GENE{{vocabulary|keywords:{{高频词1, 高频词2}}|confidence:{{HIGH/MEDIUM/LOW}}}}",
        f"  T: 高频价值词与避讳用语",
        f"  WHY: 表达印记与立场",
        f"  EVIDENCE: {{高频词和避讳表达的真实样例}}",
        f"",
        f"::GENE{{rhythm|pattern:{{段落长短规律}}|confidence:{{HIGH/MEDIUM/LOW}}}}",
        f"  T: 文本排版与节奏（如短句、列表高发、多Emoji）",
        f"  WHY: 受众阅读习惯与视觉逻辑",
        f"  EVIDENCE: {{排版格式的直接体现}}",
        f"",
        f"::GENE{{question|style:{{设问风格}}|confidence:{{HIGH/MEDIUM/LOW}}}}",
        f"  T: 设问使用习惯",
        f"  WHY: 引导互动和思考的策略",
        f"  EVIDENCE: {{真实的问句样例}}",
        f"",
        f"::GENE{{ending|style:{{结语风格}}|confidence:{{HIGH/MEDIUM/LOW}}}}",
        f"  T: 收尾/引导互动（CTA）模式",
        f"  WHY: 行为引导逻辑",
        f"  EVIDENCE: {{真实的结语/CTA 样例}}",
        f"",
        f"::GENE{{tone|perspective:{{语气基调}}|confidence:{{HIGH/MEDIUM/LOW}}}}",
        f"  T: 博主的叙事视角和语气态度（如客观中立/强烈安利/真诚陪伴）",
        f"  WHY: 认知与人设折射",
        f"  EVIDENCE: {{观点句候选中的典型语录}}",
        f"",
        f"::GENE{{audience|profile:{{目标受众}}|confidence:{{HIGH/MEDIUM/LOW}}}}",
        f"  T: 精准受众画像与痛点诉求",
        f"  WHY: 评论区数据透露的用户特征",
        f"  EVIDENCE: {{评论区典型问题的分类反馈}}",
        f"````",
        f"",
        f"---",
        f"",
...
def deep_analyze(analysis_path, nickname, output_dir, notes_details_path=None, mode="A", bio=""):
    """
    ...
    task_content = gen_distill_task(
        ...
        frequency_info, growth_info, notes,
        opinion_candidates, opinion_mode, writing_structure, value_words,
        full_notes=full_notes, mode=mode, bio=bio
    )
...
示例：
  ...
  python deep_analyze.py ./data/analysis.json "蔡不菜" -o ./output --mode B
    ...
    parser.add_argument("--bio", default="", help="博主生平/背景信息（可选，用于辅助人设认知推理）")
    ...
    result = deep_analyze(args.analysis_path, args.nickname, args.output, args.details, mode=args.mode, bio=args.bio)
